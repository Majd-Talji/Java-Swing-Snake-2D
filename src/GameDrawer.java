...
public class GameDrawer extends JPanel implements KeyListener, ActionListener {
    ...
    public void paint(Graphics g) {
        ...
        g.drawString("Developed by Majd Talji", 545, 60);
        ...
        g.drawString(totalScore + "", 550 + (142 - fm.stringWidth(bestScore + "")) / 2, 222);

        g.drawString("Fruit Score", 575, 270);
        g.drawRect(550, 280, 140, 30);
        g.drawString(fruitEaten + "", 550 + (142 - fm.stringWidth(bestScore + "")) / 2, 302);
    ...
    private void writeBestScoreFromTheFile() {
        if (totalScore >= bestScore) {
            try {
    ...
    private int readBestScoreFromTheFile() {
        try {
            FileInputStream fis = new FileInputStream("./snake-game-best-score.txt");
